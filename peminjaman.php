...
// ===== HANDLE AJAX FETCH BARANG =====
if (isset($_GET['action']) && $_GET['action'] === 'get_barang') {
    header('Content-Type: application/json; charset=utf-8');

    $kd_barang = $_GET['kd_barang'] ?? '';
    $kd_barang = trim($kd_barang);

    if ($kd_barang === '') {
        echo json_encode(['error' => 'Kode barang kosong']);
        exit;
    }

    // Prepared statement untuk keamanan
    if ($stmt = mysqli_prepare($db, "SELECT nama_barang, keterangan FROM barang WHERE kd_barang = ? LIMIT 1")) {
        ...
        if (devices && devices.length) {
          ...
          html5QrCode.start(
            ...
            qrCodeMessage => {
              ...
              const endpoint = window.location.pathname + '?action=get_barang&kd_barang=' + encodeURIComponent(kd_barang);
